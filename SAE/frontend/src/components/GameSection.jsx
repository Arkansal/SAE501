import React from 'react'
import { Link } from 'react-router-dom'
import { useTranslation } from 'react-i18next'
import './GameSection.css'
import fondtotem from '../assets/images/gametotem.svg'
import fondcode from '../assets/images/gamecode.svg'
import fondExtinction from '../assets/images/gameextinction.svg'

function GameSection({ onItemClick }) {
  const { t } = useTranslation()
// Données TEMPORAIRE ATTENTION à remplacer par des données dynamiques plus tard
  const recentGames = [
    {
      id: 1,
      name: "Ton animal totem",
      image: fondtotem,
      route: "/game-totem",
      status: "available" 
    },
    {
      id: 2,
      name: "Devine le niveau d'extinction",
      image: fondExtinction,
      route: "/extinction-game",
      status: "available",
    },
    {
      id: 3,
      name: "Code du vivant",
      image: fondcode,
      route: "/game-code-du-vivant",
      status: "available"
    },
    {
      id: 4,
      name: "Nom du jeu...",
      image: "https://www.centrepompidou.fr/_elastic_filemount/media/picture/33/7e/337e53f5f4fbd221caa223e9300aa8f4/thumb_large.jpg",
      route: "/games/game-4",
      status: "coming-soon"
    },
    {
      id: 5,
      name: "Nom du jeu...",
      image: "https://www.centrepompidou.fr/_elastic_filemount/media/picture/33/7e/337e53f5f4fbd221caa223e9300aa8f4/thumb_large.jpg",
      route: "/games/game-5",
      status: "coming-soon"
    },
    {
      id: 6,
      name: "Nom du jeu...",
      image: "https://www.centrepompidou.fr/_elastic_filemount/media/picture/33/7e/337e53f5f4fbd221caa223e9300aa8f4/thumb_large.jpg",
      route: "/games/game-6",
      status: "coming-soon"
    }
  ]

  return (
    <div className="game-section">
      <div className="game-section-header">
        <span className="game-section-icon">
         <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
            <g fill="none" fillRule="evenodd">
                <path d="m12.593 23.258l-.011.002l-.071.035l-.02.004l-.014-.004l-.071-.035q-.016-.005-.024.005l-.004.01l-.017.428l.005.02l.01.013l.104.074l.015.004l.012-.004l.104-.074l.012-.016l.004-.017l-.017-.427q-.004-.016-.017-.018m.265-.113l-.013.002l-.185.093l-.01.01l-.003.011l.018.43l.005.012l.008.007l.201.093q.019.005.029-.008l.004-.014l-.034-.614q-.005-.018-.02-.022m-.715.002a.02.02 0 0 0-.027.006l-.006.014l-.034.614q.001.018.017.024l.015-.002l.201-.093l.01-.008l.004-.011l.017-.43l-.003-.012l-.01-.01z" />
                <path fill="currentColor" d="M17 4c.763 0 1.394.434 1.856.89c.481.473.922 1.109 1.314 1.81c.787 1.406 1.472 3.243 1.925 5.058c.45 1.801.699 3.682.54 5.161C22.475 18.404 21.71 20 20 20c-1.476 0-2.652-.76-3.614-1.531l-.351-.289l-.492-.415l-.444-.368C14.08 16.572 13.175 16 12 16s-2.08.572-3.099 1.397l-.444.368l-.492.415l-.35.289C6.651 19.24 5.475 20 4 20c-1.711 0-2.476-1.596-2.635-3.081c-.158-1.48.09-3.36.54-5.161c.453-1.815 1.138-3.652 1.925-5.059c.392-.7.833-1.336 1.314-1.81C5.606 4.434 6.237 4 7 4c.515 0 1.018.123 1.513.27l.592.181q.148.046.295.087c.865.248 1.75.462 2.6.462s1.735-.214 2.6-.462l.885-.267C15.983 4.124 16.49 4 17 4m0 2c-.383 0-.783.116-1.171.243l-.458.151l-.221.068c-.885.252-2 .538-3.15.538s-2.265-.286-3.15-.538l-.22-.068l-.459-.151C7.783 6.115 7.383 6 7 6c-.418.078-.793.585-1.076 1.055l-.158.275l-.19.346c-.682 1.218-1.31 2.88-1.73 4.567c-.395 1.576-.587 3.086-.514 4.21l.026.293l.02.176l.03.208c.069.401.218.87.592.87c.812 0 1.49-.404 2.333-1.074l.403-.328l.76-.636l.344-.28C8.904 14.839 10.235 14 12 14s3.096.84 4.16 1.682l.345.28l.76.636l.402.328C18.51 17.596 19.187 18 20 18c.34 0 .494-.387.571-.759l.038-.218l.037-.317c.123-1.146-.067-2.765-.491-4.463c-.386-1.546-.946-3.072-1.562-4.254l-.359-.66l-.158-.273C17.793 6.585 17.418 6.078 17 6M8.5 8a2.5 2.5 0 1 1 0 5a2.5 2.5 0 0 1 0-5m7 0a1 1 0 0 1 .993.883L16.5 9v.5h.5a1 1 0 0 1 .117 1.993L17 11.5h-.5v.5a1 1 0 0 1-1.993.117L14.5 12v-.5H14a1 1 0 0 1-.117-1.993L14 9.5h.5V9a1 1 0 0 1 1-1m-7 2a.5.5 0 1 0 0 1a.5.5 0 0 0 0-1" />
            </g>
         </svg>
        </span>
        <h3 className="game-section-title">{t('gameSection.title')}</h3>
       
      </div>

      <div className="game-scroll-container">
        <div className="game-scroll-content">
          {recentGames.map((game) => (
            <Link
              key={game.id}
              to={game.route}
              className={`game-card ${game.status === 'coming-soon' ? 'game-coming-soon' : ''}`}
              onClick={onItemClick}
            >
              <div className="game-image-container">
                <img
                  src={game.image}
                  alt={game.name}
                  className="game-image"
                  onError={(e) => {
                    e.target.src = '/images/game-placeholder.jpg'
                  }}
                />
                 <span className="game-overlay-text">{game.name}</span>
                {game.status === 'coming-soon' && (
                  <div className="game-overlay">
                    <span className="game-overlay-text">{t('gameSection.comingSoon')}</span>
                  </div>
                )}
              </div>
              <div className="game-info">
                <h4 className="game-name">{game.name}</h4>
                <span className="game-details">{t('gameSection.details')}</span>
              </div>
            </Link>
          ))}
        </div>
      </div>
    </div>
  )
}

export default GameSection