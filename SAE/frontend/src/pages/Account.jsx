...
function CustomLanguageSelect({ value, onChange, disabled, t }) {
  ...
  const handleSelect = (langCode) => {
    if (!disabled) {
      // On met seulement à jour l'état local (tempLanguage)
      onChange(langCode)
      setIsOpen(false)
    }
  }
...
function Account() {
  ...
  // États confirmés (ceux affichés quand on n'édite pas)
  const [pseudo, setPseudo] = useState('')
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('••••••••••••••••••••••••••')
  const [language, setLanguage] = useState('fr')

  // États temporaires (ceux modifiés pendant l'édition)
    ...
    const fetchUserData = async () => {
      try {
        if (localStorage.getItem('jwt_token')) {
          console.log("Token trouvé, tentative de récupération des données utilisateur...")
          ...
          setTempLanguage(userData.language || 'fr')
        }
      } catch (err) {
  ...
  const handleEdit = () => {
    setIsEditing(true)
    // On réinitialise les valeurs temporaires avec les valeurs actuelles
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (tempLanguage !== language) {
          localStorage.setItem('i18nextLng', tempLanguage)
        ...
        alert(`${t('account.updateError')}: ${errorData['hydra:description'] || errorData.message || 'Unknown error'}`)
      }
    } catch (err) {
  ...
  if (loading) return <div className="account-container">{t('account.loading')}</div>
  if (error) return <div className="account-container">{error}</div>
      ...
      <div className="account-content">
        ...
        <div className="account-form">
          <div className="account-input-group">
            <label className="account-input-label">{t('account.pseudo')}</label>
            {isEditing ? (
              <input
                type="text"
                value={tempPseudo}
          ...
          <div className="account-input-group">
            <label className="account-input-label">{t('account.password')}</label>
            {isEditing ? (
              <>
                <input
                  type="password"
          ...
          <div className="account-input-group">
            <label className="account-input-label">{t('account.email')}</label>
            {isEditing ? (
              <input
                type="email"
                value={tempEmail}
