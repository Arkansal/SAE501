...
function CustomLanguageSelect({ value, onChange, onLanguageChange, t }) {
  ...
  const handleSelect = (langCode) => {
    onChange(langCode)
    onLanguageChange(langCode) // Change la langue globale immédiatement
    setIsOpen(false)
  }
...
function Register() {
  ...
  // Change la langue globale immédiatement
  ...
  const handleSubmit = async (e) => {
    ...
    if (password.length < 8) {
      setError(t('register.passwordMinLength') || 'Le mot de passe doit contenir au moins 8 caractères')
      return
    }

    if (!acceptTerms) {
      setError(t('register.acceptTerms') || 'Vous devez accepter les conditions d\'utilisation')
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(t('register.success') || `Compte créé avec succès ! ID: ${data.id}`)
        ...
        if (data.errors) {
          ...
          setError(data.error || t('register.error') || 'Une erreur est survenue lors de la création du compte')
        }
      }
    } catch (err) {
      setError(t('register.networkError') || 'Erreur de connexion au serveur. Veuillez réessayer.')
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="register-content">
        ...
        <form onSubmit={handleSubmit} className="register-form">
          ...
          <div className="register-checkbox-group">
            ...
            />
            <label htmlFor="terms">
              {t('register.agreeTerms')} <span className="register-terms-link">{t('register.termsLink')}</span>
            </label>
          </div>
          ...
          <button 
            ...
            disabled={isLoading}
          >
            {isLoading ? t('register.loading') || 'Création en cours...' : t('register.submit')}
          </button>
        </form>
